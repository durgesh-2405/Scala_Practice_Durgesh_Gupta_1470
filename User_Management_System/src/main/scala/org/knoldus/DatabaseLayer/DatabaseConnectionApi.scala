...
trait DatabaseConnectionApi {
  ...
  def insertUserDatabase(user: User):String= {
    try {
      ...
      statement.executeUpdate(s"INSERT INTO userdatabase.user VALUES ('$id', '$name','$userType')")
      "Insertion performed"
    }
    catch {
  ...
  def selectUser(): String = {
    try {
      ...
      connection = DriverManager.getConnection(url, username, password)
      val statement = connection.createStatement()
      val resultSet = statement.executeQuery("SELECT * FROM userdatabase.user")
      print("\nDATA FROM DATABASE: \n")
  ...
  def getUserNameByIdDatabase(id: Int): String = {
    try {
      ...
      val statement = connection.createStatement()
      val Id = id
      val resultSet = statement.executeQuery(s"SELECT Name FROM userdatabase.user where id = '$Id'")
  ...
  def updateUserDatabase(id: Int, new_name: String): String = {
    try {
      ...
      statement.executeUpdate(s"UPDATE userdatabase.user SET name = '$newName' where id = '$idToChange'")
      "Name updated"
    }
    catch {
      case e: Throwable=> e.printStackTrace()
  ...
  def deleteUserDatabase(id : Int) : String = {
    try {
      ...
      val statement = connection.createStatement()
      val idToDelete = id
      statement.executeUpdate(s"DELETE from userdatabase.user where id = '$idToDelete'")
      "Row deleted"
    }
    catch {
      case e: Throwable => e.printStackTrace()
